...
error_detect_depends(){
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "Please visit: https://t.me/siemenstutorials"
        exit 1
    fi
}
...
pre_install_caddy(){
    ...
    read -p "(Default path: /support/microsoft.com):" v2ray_path
    [ -z "${v2ray_path}" ] && v2ray_path="/support/microsoft.com"
    echo
    echo "---------------------------"
...
config_docker(){
    ...
    curl -L https://raw.githubusercontent.com/siemenstutorials/v2rays/master/docker-compose.yml > docker-compose.yml
...
config_caddy_docker(){
    ...
    echo "install curl"
    install_dependencies
    curl -L https://raw.githubusercontent.com/siemenstutorials/v2rays/master/Caddy_V2ray/Caddyfile >  Caddyfile
    echo "Writing docker-compose.yml"
    curl -L https://raw.githubusercontent.com/siemenstutorials/v2rays/master/Caddy_V2ray/docker-compose.yml > docker-compose.yml
    sed -i "s|node_id:.*|node_id: ${ssrpanel_node_id}|"  ./docker-compose.yml
    sed -i "s|sspanel_url:.*|sspanel_url: '${ssrpanel_url}'|"  ./docker-compose.yml
    sed -i "s|key:.*|key: '${ssrpanel_key}'|"  ./docker-compose.yml
    sed -i "s|speedtest:.*|speedtest: ${ssrpanel_speedtest}|"  ./docker-compose.yml
